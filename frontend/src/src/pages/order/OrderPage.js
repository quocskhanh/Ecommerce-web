...
// Confirm and delete selected orders
    const handleConfirmDelete = async () => {
        try {
            for (const id of selectedOrders) {
                await fetch(`${ecommerceAPI.baseURL}orders/${id}`, {
                    method: "DELETE",
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem("access_token")}`,
                        "Content-Type": "application/json",
 ...
 // Handle checkbox selection
    ...
    const fetchOrderDetails = async (orderId) => {
        try {
            const response = await fetch(`${ecommerceAPI.baseURL}orders/${orderId}/items`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("access_token")}`,
                },
            });
            ...
            const detailedItems = await Promise.all(
                items.map(async (item) => {
                    const productResponse = await fetch(`${ecommerceAPI.baseURL}products/${item.product_id}`, {
                        headers: {
                            Authorization: `Bearer ${localStorage.getItem("access_token")}`,
                        },
                    });
